	...
	return (
		<div className="overflow-hidden h-[100vh]">
			<div className={`w-[98vw] flex items-center justify-center duration-1000 ${isInView ? "opacity-100" : "translate-y-[300px] opacity-0"} transition-all`} ref={ref}>
				{ranking && (
					<div className="flex flex-col items-center h-screen w-[400px] sm:w-[600px]">
							...
							<div className="flex flex-col items-center h-4/5 bg-white bg-opacity-50 rounded-xl p-4 space-y-3">
								...
								{ranking[1] && (
									<>
										...
										<p className="text-sm bg-white text-center rounded-lg py-2 px-4">{ranking[1].totalPlasticAmount.toFixed(2)}</p>
									</>
								)}
							</div>
							<div className="flex flex-col items-center h-full bg-white bg-opacity-50 rounded-xl p-4 space-y-3">
								...
								{ranking[0] && (
									<>
										...
										<p className="text-sm bg-white text-center rounded-lg py-2 px-4">{ranking[0].totalPlasticAmount.toFixed(2)}</p>
									</>
								)}
							</div>
							<div className="flex flex-col items-center h-4/5 bg-white bg-opacity-50 rounded-xl p-4 space-y-3">
								...
								{ranking[2] && (
									<>
										...
										<p className="text-sm bg-white text-center rounded-lg py-2 px-4">{ranking[2].totalPlasticAmount.toFixed(2)}</p>
									</>
								)}
							</div>
						</div>
									...
									i >= 3 && (
										<li key={`rank${i}`} className="flex items-center p-4 space-x-4">
											...
											<div>
												...
												<p className="text-sm">{user.totalPlasticAmount.toFixed(2)}</p>
											</div>
										</li>
									)
							)}
